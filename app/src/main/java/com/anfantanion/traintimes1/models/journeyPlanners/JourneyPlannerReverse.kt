...
class JourneyPlannerReverse(
    ...
    var lastTimeDate = TimeDate(startTime=endTime)
    ...
    fun prevStation(){
        if (index < 1){
            serviceStubs.reverse()
            journeyListener(serviceStubs)
            return
        }
        var temp = lastTimeDate.copy()
        temp.addMinutes(-50)
        val filter = RTTAPI.Filter(
            from = waypoints[index - 1].crs,
            to = null,
...
//        )
    ...
    fun serviceResponseList(response: List<ServiceResponse>) {
        ...
        val times = response
            .filter { sr -> TimeDate(startTime = sr.getStationArrival(waypoints[index+1])).calendar.timeInMillis < lastTimeDate.calendar.timeInMillis }//Only services arriving before deadline
            .sortedBy{ sr -> TimeDate(startTime = sr.getStationDeparture(waypoints[index])).calendar.timeInMillis } //Sort by time
        ...
        val departureTime = fastestService.getRTorTTDeparture(waypoints[index])
        ...
        lastTimeDate.setTime(departureTime)
        lastTimeDate.addMinutes(-allowChangeTimeOf)
        prevStation()
    }
